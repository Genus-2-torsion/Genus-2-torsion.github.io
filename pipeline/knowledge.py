...
SOURCES = {
    "BNSS2026": {
        "short": "Balakrishnan–Najman–Shnidman–Sutherland 2026", "year": 2026,
    ...
    "Genus2Torsion": {
        "short": "Genus2Torsion repository", "year": 2026,
    ...
    "Najman2026": {
        "short": "Najman 2026 (unpublished)", "year": 2026,
    ...
    "BSSVY": {
        "short": "LMFDB (BSSVY 2016)", "year": 2016,
    ...
    "BookerSutherland": {
        "short": "Booker–Sutherland", "year": 2026,
...
assert all("year" in v for v in SOURCES.values()), [k for k, v in SOURCES.items() if "year" not in v]


def first_source(keys):
    """The earliest-dated source among the keys (as credited in the paper's tables), or None."""
    ks = [k for k in keys if k in SOURCES]
    return min(ks, key=lambda k: SOURCES[k]["year"]) if ks else None
